...
async function main() {
  ...
  const asset1 = await prisma.investmentAsset.create({ data: { walletId: investmentWallet.id, assetType: AssetType.REKSADANA, subType: 'Pasar Uang', assetName: 'Reksadana Pasar Uang', totalInvested: 2600000, totalValue: 2685000, createdAt: months[4] } });
  const asset2 = await prisma.investmentAsset.create({ data: { walletId: investmentWallet.id, assetType: AssetType.EMAS, assetName: 'Tabungan Emas', totalInvested: 1800000, totalValue: 1940000, createdAt: months[5] } });
